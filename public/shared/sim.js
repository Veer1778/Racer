...
export function pitGeometry(built) {
  ...
  // The lane has to sit INSIDE the barrier. Put it beyond the barrier and the
  // wall blocks every attempt to enter, which is exactly how the pit lane
  // looked to a driver before: painted on the far side of a wall.
  const inner = built.width / 2 + (built.runoff || 6) - 3.6;
  room = Math.max(built.width / 2 + 3.5, Math.min(room, inner));

  built._pit = {
    entry, exit,
    box: entry + Math.max(40, (back * step) * 0.55),
    offset: -room,
    usable: back * step > 120 && room > built.width / 2 + 3.4
  };
  return built._pit;
}
...
export function stepCar(built, car, input, drv, dt, raceTime, events) {
  ...
  // Simply driving into the lane counts as entering it, so the pit lane is
  // discoverable without knowing the key. Only near the entry though, and not
  // on a lap where a stop was already served: a car rejoining the circuit
  // crosses this same band on the way out and would be dragged straight back in.
  const pastEntry = dNow >= pit.entry ? dNow - pit.entry : (built.length - pit.entry) + dNow;
  // the gate sits clear of the track edge, so merely running wide onto the
  // runoff on the main straight never counts as entering the pits
  const laneGate = Math.min(pit.offset + 2.2, -(half + 2.6));
  if (car.pit === 'no' && pit.usable && pastEntry < 150 && car.pitLap !== car.lap
      && car.lateral < laneGate && car.lateral > pit.offset - 4.0) {
    car.pit = 'lane';
    fire('pit', { state: 'lane' });
  }
  const inLane = car.pit === 'lane' || car.pit === 'stopped';
  ...
  if (car.pit === 'stopped') {
    ...
    if (car.pitClock <= 0) {
      ...
      car.pit = 'lane';
      car.served = true;
      car.pitLap = car.lap;          // one stop per lap, however the car moves
  ...
  const limit = half + (built.runoff || TUNE.wallMargin);
